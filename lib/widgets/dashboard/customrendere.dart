...
class BarLabelDecoratorWorkAround<D> extends BarRendererDecorator<D> {
  ...
  static final _defaultInsideLabelStyle = TextStyleSpec(fontSize: 12, color: Color.white);
  static final _defaultOutsideLabelStyle = TextStyleSpec(fontSize: 12, color: Color.black);
  ...
  @override
  void decorate(Iterable<ImmutableBarRendererElement<D>> barElements, ChartCanvas canvas, GraphicsFactory graphicsFactory,
      {required Rectangle<int> drawBounds, required double animationPercent, required bool renderingVertically, bool rtl = false}) {
  ...
  void _decorateVerticalBars(Iterable<ImmutableBarRendererElement<D>> barElements, ChartCanvas canvas, GraphicsFactory graphicsFactory,
      Rectangle<int> drawBounds, bool rtl) {
    ...
    for (var element in barElements) {
      ...
      final datumInsideLabelStyle =
          _getDatumStyle(element.series!.insideLabelStyleAccessorFn, datumIndex, graphicsFactory, defaultStyle: insideLabelStyle);
      final datumOutsideLabelStyle =
          _getDatumStyle(element.series!.outsideLabelStyleAccessorFn, datumIndex, graphicsFactory, defaultStyle: outsideLabelStyle);
      ...
      var labelElements = label.split(_labelSplitPattern).map((labelPart) => graphicsFactory.createTextElement(labelPart));
      ...
      if (calculatedLabelPosition == BarLabelPosition.auto) {
        ...
        labelElements = labelElements.map((labelElement) => labelElement..textStyle = datumInsideLabelStyle);

        final labelMaxWidth = labelElements
            .map((labelElement) => labelElement.measurement.horizontalSliceWidth)
        ...
        calculatedLabelPosition =
            totalLabelHeight < insideBarHeight && labelMaxWidth < bounds.width ? BarLabelPosition.inside : BarLabelPosition.outside;
      }
      ...
      for (var labelElement in labelElements) {
        ...
        final offsetHeight = (labelHeight + _defaultMultiLineLabelPadding) * labelsDrawn;

        if (calculatedLabelPosition == BarLabelPosition.inside) {
          final anchor = _resolveLabelAnchor(measure, labelAnchor ?? _defaultVerticalLabelAnchor);
          switch (anchor) {
            case BarLabelAnchor.end:
              labelY = bounds.top + labelPadding + offsetHeight;
              break;
            case BarLabelAnchor.middle:
              labelY = (bounds.bottom - bounds.height / 2 - totalLabelHeight / 2 + offsetHeight).round();
              break;
            case BarLabelAnchor.start:
              labelY = bounds.bottom - labelPadding - totalLabelHeight + offsetHeight;
              break;
          }
        } else {
  ...
  void _decorateHorizontalBars(Iterable<ImmutableBarRendererElement<D>> barElements, ChartCanvas canvas, GraphicsFactory graphicsFactory,
      Rectangle<int> drawBounds, bool rtl) {
    // Create [TextStyle] from [TextStyleSpec] to be used by all the elements.
    // The [GraphicsFactory] is needed so it can't be created earlier.
    final insideLabelStyle = _getTextStyle(graphicsFactory, insideLabelStyleSpec);
    ...
    for (var element in barElements) {
      ...
      if (calculatedLabelPosition == BarLabelPosition.auto) {
        ...
        calculatedLabelPosition = (insideBarWidth >= outsideBarWidth || labelElement.measurement.horizontalSliceWidth < insideBarWidth)
      ...
      if ((labelElement.maxWidth! < 0 ||
              (labelElement.maxWidthStrategy == null && labelElement.measurement.horizontalSliceWidth > labelElement.maxWidth!)) &&
      ...
      final int labelX;
      if (calculatedLabelPosition == BarLabelPosition.inside) {
        final anchor = _resolveLabelAnchor(measure, labelAnchor ?? _defaultHorizontalLabelAnchor);

        switch (anchor) {
          case BarLabelAnchor.middle:
            labelX = (bounds.left + bounds.width / 2 - labelElement.measurement.horizontalSliceWidth / 2).round();
            labelElement.textDirection = rtl ? TextDirection.rtl : TextDirection.ltr;
            break;

          case BarLabelAnchor.end:
          case BarLabelAnchor.start:
            final alignLeft = rtl ? (anchor == BarLabelAnchor.end) : (anchor == BarLabelAnchor.start);
        ...
        if (measure < 0) {
          labelX = (bounds.right - labelElement.measurement.horizontalSliceWidth).round();
        } else {
          labelX = (bounds.left + drawBounds.width - labelElement.measurement.horizontalSliceWidth).round();
        }
      }
      ...
      if (labelVerticalPosition == BarLabelVerticalPosition.middle) {
        // Center the label inside the bar.
        labelY = (bounds.top + (bounds.bottom - bounds.top) / 2 - labelElement.measurement.verticalSliceWidth / 2).round();
      } else {
        /// labelVerticalPosition == BarLabelVerticalPosition.top
        labelY = (bounds.top - labelElement.measurement.verticalSliceWidth - labelPadding).round();
      }
  ...
  int _getTotalLabelHeight(Iterable<TextElement> labelElements) =>
      (labelElements.first.measurement.verticalSliceWidth * labelElements.length).round() +
